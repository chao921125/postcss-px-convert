...
describe('px2any 基础功能', () => {
  ...
  it('小于 minPixelValue 不转换', () => {
    ...
    const result = px2any(css, { unitToConvert: 'rem', rootValue: 16, minPixelValue: 1 });
  ...
  it('selectorBlackList 不转换', () => {
    ...
    const fakeRoot = {
      walkRules: (cb: any) => {
        cb({ selector: '.ignore', walkDecls: (cb2: any) => {
          const decl: any = { prop: 'font-size', _v: '20px' };
        ...
        cb({ selector: '.ok', walkDecls: (cb2: any) => {
          const decl: any = { prop: 'font-size', _v: '20px' };
          Object.defineProperty(decl, 'value', {
            get() { return this._v; },
            set(v) { this._v = v; }
  ...
  it('propList 只转换指定属性', () => {
    ...
    const result = px2any(css, { unitToConvert: 'rem', rootValue: 20, propList: ['font-size'] });
  ...
  it('isFileIncluded include/exclude 文件路径过滤', () => {
    const { isFileIncluded } = require('../src/utils');
